...
export default function KaraokePage() {
  ...
  const [searchSource, setSearchSource] = useState<'youtube' | 'local'>('youtube');
  ...
  const handleSearch = async () => {
    ...
    try {
      if (searchSource === 'youtube') {
        ...
        const data = await response.json();
        if (!response.ok) throw new Error(data.error || 'Failed to search local music');
        const filtered = data.files.filter((file: any) => file.name.toLowerCase().includes(searchQuery.toLowerCase()));
        setSearchResults(filtered.map((file: any) => ({
          id: file.path,
          title: file.name,
  ...
  const handleNextSong = () => {
    ...
    if (currentIndex < selectedSongs.length - 1) {
      const nextSong = selectedSongs[currentIndex + 1];
      setCurrentSong(nextSong);
      setIsPlaying(true);
      toast({
        title: 'Next Song',
        description: `Next: ${nextSong.title}`,
        duration: 4000,
      });
    } else {
      setCurrentSong(null);
            ...
            <Card className="p-4">
              <Tabs value={activeTab} onValueChange={setActiveTab}>
                ...
                <TabsContent value="search" className="mt-4">
                  <Tabs value={searchSource} onValueChange={v => setSearchSource(v as 'youtube' | 'local')} className="mb-2">
                    <TabsList>
                  ...
                  <div className="mt-4 space-y-2 max-h-[600px] overflow-y-auto">
                    {searchResults.map((song) => {
                      ...
                      return (
                        <Card
                          key={song.id}
                          className={`p-2 hover:bg-accent transition-colors ${isInQueue ? 'border-primary' : ''} cursor-pointer`}
                          onClick={() => {
                            if (isInQueue) {
                              setCurrentSong(song);
                              setIsPlaying(true);
                        ...
                        >
                          <div className="flex items-center space-x-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <span className="text-xs px-2 py-0.5 rounded bg-gray-200 text-gray-600 ml-1">{song.source}</span>
                            </div>
                            <Button
                              variant="ghost"
                              size="sm"
